 ...
 * RETURNS: None.
 */
static VOID AL5_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("AL5{SHAD}30.LOG", "a")) != NULL)
  {
    fprintf(F, "%s: %s\n", Stage, Text);
    fclose(F);
  }
 ...
 */
static CHAR * AL5_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  CHAR *text;
  INT flen;
...
UINT AL5_RndShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      AL5_RndShaderLog(suff[i], "Error create shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    txt = AL5_RndShaderLoadTextFile(buf);
    if (txt == NULL)
    {
      AL5_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'AL5_RndShaderLoad' function */
...
VOID AL5_RndShaderFree( UINT Prg )
{
  ...
  glDeleteProgram(Prg);
} /* End of 'AL5_RndShaderFree' function */
